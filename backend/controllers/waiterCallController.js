...
const resolveWaiterCall = async (req, res) => {
    try {
        const { id } = req.params;
        const resolvedStatus = await WaiterCallSchema.findByIdAndUpdate(id, { status: "resolved" }, { returnDocument: 'after', runValidators: true })
        res.status(200).json(resolvedStatus)
    } catch (error) {
        res.status(500).json({message : "failed to update the status"})
    }
}

module.exports = { createWaiterCall, getAllWaiterCalls, resolveWaiterCall }